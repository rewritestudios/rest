...
import { FetchOptions, RESTOptions, RewriteHandleErrorOptions } from "./types";
import { backoff, createURL, FIVE_SECONDS_IN_MS, isRetryableStatus, sleep } from "./utils";

export class REST {
	public options: RESTOptions;
	public headers = { 'Content-Type': 'application/json' } as Record<string, string>;
	...
	private async fetch<R>(route: string, options: FetchOptions, attempt = 0) {
		...
		if (!response.ok) return this.handleError<R>({ route, attempt, options, response, method: options.method });
	...
	private async handleError<R>({ route, method, options, attempt, response }: RewriteHandleErrorOptions): Promise<R> {
		...
		if (attempt >= (retry?.max ?? 3)) throw new HTTPError('Max retries reached', response.status, response.url, method);
		if (retry?.onRetry) await retry.onRetry({ route, attempt, options, response, method });
